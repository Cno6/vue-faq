...
<template>
  <div class="todo-container w3-white w3-card-4">
    ...
    <div class="w3-container flex-container w3-light-gray w3-padding">
      <input type="text" class="w3-input w3-border-0" autofocus v-model="todoText" @keyup.enter="addTodo" placeholder="Type here your todo-item...">
    ...
    <div class="w3-padding w3-blue">Completed ({{ done.length }})</div>
    <div class="w3-padding" v-for="todo in done" :key="todo.id">
      <label>
        <input type="checkbox" v-model="todo.checked" checked>
        <span class="w3-margin-left">
          {{ todo.text }}
        </span>
      </label>
